...
public interface PrivilegeRepo extends JpaRepository<Privilege, Long> {
    ...
    @Query("select t from Privilege t where t.activated =:activated")
    Page<Privilege> getAllByActivated(@Param("activated") boolean activated, Pageable pageable);

    @Query("select t from Privilege t")
    Page<Privilege> getAll(Pageable pageable);

    @Query("select t from Privilege t where t.activated = :activated ")
    List<Privilege> getAllByActivated(@Param("activated") boolean activated);
